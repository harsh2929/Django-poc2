...
def like_view(request, pk):
    ...
    if post.likes.filter(id=request.user.id).exists():
        ...
        liked = True
    return HttpResponseRedirect(reverse('qabase:question-detail', args=[str(pk)]))
...
class CommentDetailView(CreateView):
    ...
    template_name = 'qabase/question-detail.html'
    
    def form_valid(self, form):
        form.instance.question_id = self.kwargs['pk']
        return super().form_valid(form)
    success_url = reverse_lazy('qabase:question-detail')

class AddCommentView(CreateView):
    model = Comment
    form_class = CommentForm
    
    template_name = 'qabase/question-answer.html'

    def form_valid(self, form):
        form.instance.question_id = self.kwargs['pk']
        return super().form_valid(form)
    success_url = reverse_lazy('qabase:question-lists')
